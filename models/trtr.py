...
class TRTR(nn.Module):
    ...
    def __init__(self, backbone, heatmap_transformer, bbox_transformer, aux_loss=False, transformer_mask=False):
        ...
        """
        super().__init__()
        self.transformer_heatmap = nn.ModuleList([copy.deepcopy(heatmap_transformer) for i in backbone.num_channels_list]) # multiple heamtmap transformer for layer-wise backbone output
        self.transformer_bbox = nn.ModuleList([copy.deepcopy(bbox_transformer) for i in backbone.num_channels_list]) # multiple bbox transformer for layer-wise backbone output

        heatmap_hidden_dim = heatmap_transformer.d_model
        bbox_hidden_dim = bbox_transformer.d_model

        # heatmap and bbox
        # TODO: try to use MLP or Conv2d_3x3 before fully-connected like CenterNet
        backbone_layer_num = len(backbone.num_channels_list)
        self.bbox_embed = MLP(backbone_layer_num * bbox_hidden_dim, backbone_layer_num * bbox_hidden_dim, 4, 3)
        self.heatmap_embed = nn.Linear(backbone_layer_num * heatmap_hidden_dim, 1)
        self.heatmap_embed.bias.data.fill_(-2.19)

        self.heatmap_input_projs = nn.ModuleList([nn.Conv2d(num_channels, heatmap_hidden_dim, kernel_size=1) for num_channels in backbone.num_channels_list])
        self.bbox_input_projs = nn.ModuleList([nn.Conv2d(num_channels, bbox_hidden_dim, kernel_size=1) for num_channels in backbone.num_channels_list])
...
def build(args):
    ...
    heatmap_transformer = build_transformer(args)
    args_copy = copy.deepcopy(args)  # TODO: force to use default transformer for bbox regression
    args_copy.use_default_transformer = True
    bbox_transformer = build_transformer(args_copy)
