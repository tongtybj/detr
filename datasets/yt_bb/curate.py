...
def image_curation(num_threads=24):
    ...
    for d_set in d_sets:
        ...
        print("Cropping images on {} videos for d_set".format(len(vids)))
        ...
        with futures.ProcessPoolExecutor(max_workers=num_threads) as executor:
            fs = [executor.submit(crop_video, vid) for vid in vids]
            for i, f in enumerate(futures.as_completed(fs)):
                sys.stderr.write("Curate video: {} / {} \r".format(i, len(vids)))

        for i, vid in enumerate(vids):

            sys.stderr.write("dumping video {} / {} in {} \r".format(i, len(vids), d_set))
            image_w = 0
            image_h = 0
